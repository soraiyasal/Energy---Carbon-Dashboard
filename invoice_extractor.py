...
def main():
    st.set_page_config(page_title="Energy & Carbon Dashboard", layout="wide")
...
#                     {
#                         "role": "system",
#                         "content": """You are a precise invoice data extraction assistant. 
#                         Extract the following from energy invoices and return ONLY a valid JSON object:
#                         - Total kWh usage (sum of day and night if applicable)
#                         - Billing period dates
#                         - Provider name
#                         - Energy type (determine if it's a gas or electricity invoice)"""
#                     },
#                     {
#                         "role": "user",
#                         "content": f"""Extract these fields from the invoice and return them in this exact JSON format:
...
#                             "billing_period_end": "DD/MM/YYYY",
#                             "provider": "provider_name",
#                             "type": "electricity" or "gas"
#                         }}

#                         For day/night tariffs, sum the kWh values.
#                         Format all dates as DD/MM/YYYY.
#                         If any field is not found, use null.
#                         Default to "electricity" if energy type is unclear.
...
#                 for field in required_fields:
#                     if field not in result:
#                         result[field] = 'electricity' if field == 'type' else None
#                 return result
            
#             except json.JSONDecodeError as e:
#                 st.error(f"Invalid JSON response from OpenAI: {content}")
#                 return { 'kwh': None, 'billing_period_start': None, 'billing_period_end': None, 'provider': None, 'type': 'electricity' }
...
#             result = InvoiceProcessor.extract_invoice_data(redacted_text)
#             if result:
#                 result['filename'] = uploaded_file.name
#             return result
...
# class CarbonCalculator:
#     @staticmethod
#     def get_factor(year, energy_type):
#         """Get DEFRA conversion factor for the given year and energy type."""
#         return ELECTRICITY_FACTORS.get(year) if energy_type == 'electricity' else GAS_FACTORS.get(year)
...
# class Dashboard:
#     def __init__(self):
#         self.chart_type = st.sidebar.selectbox("Select Chart Type", ["Line Chart", "Bar Chart", "Area Chart"])

#     def create_usage_chart(self, df):
#         """Generate energy usage trend chart based on user-selected chart type."""
#         if self.chart_type == "Line Chart":
#             return px.line(df, x='billing_period_start', y='kwh', color='type', title='Energy Usage Trend', markers=True)
#         elif self.chart_type == "Bar Chart":
#             return px.bar(df, x='billing_period_start', y='kwh', color='type', title='Energy Usage Trend')
#         else:
#             return px.area(df, x='billing_period_start', y='kwh', color='type', title='Energy Usage Trend')

#     def create_emissions_charts(self, df):
#         """Create emissions-related visualizations."""
#         emissions_trend = px.line(
#             df, x='billing_period_start', y='emissions_tonnes', color='type', title='Monthly Carbon Emissions', markers=True
#         )
#         emissions_vs_usage = px.scatter(
#             df, x='kwh', y='emissions_tonnes', color='type', title='Energy Usage vs Carbon Emissions'
#         )
#         return emissions_trend, emissions_vs_usage

#     def display_dashboard(self, df, carbon_metrics):
#         """Display dashboard metrics, charts, and recommendations."""
...
#         # Display emissions metrics for electricity and gas separately
#         st.subheader("Emissions Breakdown")
#         col1, col2 = st.columns(2)
#         with col1:
#             st.metric("Scope 1 Emissions (Gas)", f"{carbon_metrics['scope1_emissions_tonnes']:,.1f} tCO2e")
...
#         # Create tabs
#         tab1, tab2, tab3, tab4 = st.tabs(["Usage & Emissions", "Monthly Comparison", "Environmental Impact", "Raw Data"])
...
#     def display_monthly_comparison(self, df):
#         """Display monthly usage and emissions comparison chart, separated by Scope 1 and Scope 2."""
#         monthly_df = df.copy()
#         monthly_df['month_year'] = monthly_df['billing_period_start'].dt.strftime('%B %Y')
        
#         st.subheader("Monthly Comparison by Scope")

#         for scope, scope_type in [("Scope 1 (Gas)", "gas"), ("Scope 2 (Electricity)", "electricity")]:
#             scope_df = monthly_df[monthly_df['type'] == scope_type]

#             # Sort by chronological order for visual clarity
#             scope_df = scope_df.sort_values('billing_period_start')

#             # Monthly usage comparison for each scope
#             monthly_fig = go.Figure()
#             monthly_fig.add_trace(go.Bar(
#                 x=scope_df['month_year'],
#                 y=scope_df['kwh'],
#                 name=f"{scope} Monthly Usage",
#                 text=scope_df['kwh'].round(2),
#                 textposition='auto'
#             ))
#             monthly_fig.update_layout(
#                 title=f"{scope} Monthly Usage Comparison",
#                 xaxis_title='Month',
#                 yaxis_title='Usage (kWh)',
#                 xaxis={'tickangle': 45}
#             )
...
#             # Display monthly statistics for kWh and emissions for each scope
#             monthly_stats = scope_df.groupby('month_year').agg({
#                 'kwh': ['sum', 'mean'],
#                 'emissions_tonnes': ['sum', 'mean']
#             }).round(2)
...
#                 f'Average kWh ({scope})', 
#                 f'Total tCO2e ({scope})', 
#                 f'Average tCO2e ({scope})'
#             ]
            
#             st.write(f"{scope} Monthly Statistics")
#             st.dataframe(monthly_stats)


#     def display_environmental_impact(self, df):
#         """Display environmental impact metrics and equivalents."""
#         total_emissions = df['emissions_tonnes'].sum()
        
#         TREE_ABSORPTION_RATE = 0.01  # tonnes CO2 per tree per year
#         CAR_EMISSIONS_PER_KM = 0.142  # kg CO2 per km
#         HOME_ANNUAL_EMISSIONS = 2.7  # tonnes CO2 per home per year
...
#         col1, col2 = st.columns(2)
#         with col1:
#             st.metric("Trees Needed for Offset", f"{trees_needed:,d} trees")
#             st.metric("Equivalent Car Travel", f"{car_km:,d} km")
#         with col2:
#             st.metric("Homes Annual Electricity Usage Equivalent", f"{homes_equivalent:,d} homes")

#     def display_raw_data(self, df):
#         """Display raw data and provide a download option, separated by Scope 1 and Scope 2."""
#         st.subheader("Raw Data by Scope")
        
#         for scope, scope_type in [("Scope 1 (Gas)", "gas"), ("Scope 2 (Electricity)", "electricity")]:
#             scope_df = df[df['type'] == scope_type]
#             st.write(f"{scope} Raw Data")
#             st.dataframe(scope_df[['filename', 'billing_period_start', 'kwh', 'emissions_tonnes', 'carbon_factor']].round(2))

#             csv = scope_df.to_csv(index=False)
#             st.download_button(
#                 label=f"📥 Download {scope} Data (CSV)",
#                 data=csv,
#                 file_name=f"{scope_type}_data.csv",
#                 mime="text/csv"
#             )
...
# def main():
#     st.title("Energy & Carbon Dashboard")
#     input_method = st.radio("Select Input Method", ["Upload PDFs", "Manual Input", "Sample Data"], horizontal=True)
...
#         for i in range(n_months):
#             with st.expander(f"Month {i+1}", expanded=True):
#                 month_date = st.date_input(f"Select Date for Month {i+1}", key=f"date_{i}")
#                 elec_kwh = st.number_input(f"Electricity Usage (kWh) for Month {i+1}", min_value=0.0, value=0.0, key=f"elec_kwh_{i}")
#                 gas_kwh = st.number_input(f"Gas Usage (kWh) for Month {i+1}", min_value=0.0, value=0.0, key=f"gas_kwh_{i}")
                
#                 # Append electricity and gas entries
#                 if elec_kwh > 0:
#                     manual_data.append({
#                         'filename': f'manual_elec_{i+1}',
#                         'kwh': elec_kwh,
#                         'billing_period_start': month_date,
#                         'type': 'electricity'
#                     })
...
#             df = pd.DataFrame(manual_data)
#             df['billing_period_start'] = pd.to_datetime(df['billing_period_start'])
#             df['billing_period_end'] = df['billing_period_start'] + pd.DateOffset(months=1) - pd.DateOffset(days=1)
#             carbon_metrics = CarbonCalculator.calculate_metrics(df)
#             dashboard.display_dashboard(df, carbon_metrics)
...
#     else:  # PDF Upload
#         st.write("Upload your electricity invoices to analyze energy usage and carbon emissions.")
#         uploaded_files = st.file_uploader("Choose PDF files (maximum 10)", type="pdf", accept_multiple_files=True)
#         if uploaded_files:
#             data = [InvoiceProcessor.process_pdf(file) for file in uploaded_files if InvoiceProcessor.process_pdf(file)]
#             if data:
#                 df = pd.DataFrame(data)
#                 df['billing_period_start'] = pd.to_datetime(df['billing_period_start'])
#                 df = df.sort_values('billing_period_start')
...
#     """Load sample data for the last 12 months with distinct monthly patterns for electricity and gas."""
#     sample_data = []
#     start_date = datetime.now().replace(day=1) - pd.DateOffset(months=12)
    
#     # Define monthly usage patterns for electricity and gas
#     electricity_patterns = [12000, 11500, 11000, 10500, 10000, 9500, 9000, 8500, 8000, 8500, 9000, 9500]  # kWh values
#     gas_patterns = [7000, 7500, 7200, 7100, 7000, 6900, 6800, 6700, 6600, 6700, 6800, 6900]  # kWh values
...
#         sample_data.append({
#             'filename': f'sample_electricity_data_{i+1}',
#             'kwh': electricity_patterns[i],
#             'billing_period_start': date.strftime('%d/%m/%Y'),
#             'billing_period_end': (date + pd.DateOffset(months=1) - pd.DateOffset(days=1)).strftime('%d/%m/%Y'),
#             'type': 'electricity'
...
#         # Gas data entry
#         sample_data.append({
#             'filename': f'sample_gas_data_{i+1}',
#             'kwh': gas_patterns[i],
#             'billing_period_start': date.strftime('%d/%m/%Y'),
#             'billing_period_end': (date + pd.DateOffset(months=1) - pd.DateOffset(days=1)).strftime('%d/%m/%Y'),
#             'type': 'gas'
